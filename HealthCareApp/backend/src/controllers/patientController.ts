...
export const getPatients = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = (req as any).user;
    const workerUid = user.uid;
    // Health workers see their own created patients, or all if we don't filter.
    // For now, let's return all patients created by this health worker for safety.
    const queryStr = req.query.query as string || '';
    
    let q = db.collection(PATIENTS_COLLECTION).where('createdBy', '==', workerUid);
    const snap = await q.get();

    let patients = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
